...
def run_experiment(cfg):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(x_combined)):
        ...
        if cfg.task.dataset in ['mnist', 'fashionmnist']:
            ...
            print(x_train_fold.mean())
            x_test_fold = scaler.transform(x_test_fold)
            print(x_test_fold.mean())
        ...
        if cfg.task.task_type == 'regression':
            ...
            print(f"Test MSE: {test_MSE:.4f}, Test RMSE: {test_rmse:.4f}, Test  NLL: {test_nll}, Avg Time / Epoch: {avg_train_time_per_epoch:.4f} ")
