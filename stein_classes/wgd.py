...
def apply_WGD(modellist, parameters, batch, train_dataloader, 
    ...
    # print("ensemble_parameters_tensor",ensemble_parameters_tensor.shape)
    # print("squared_distances.shape:",squared_distances.shape)
    # print("grad_K.shape:",grad_K.shape)
    
    # print("score_func_tensor:",score_func_tensor.shape)
    # print("torch.einsum('mn, mo -> no', K_XX, score_func_tensor) / n_particles",(torch.einsum('mn, mo -> no', K_XX, score_func_tensor) / n_particles).shape)
    # print("")

    # svgd = -1 * torch.einsum('mn, mo -> no', K_XX, score_func_tensor) / n_particles + torch.mean(grad_K, dim=0)    
    
    # term to add to the gradient
    # print("torch.sum(grad_K,dim=0)",torch.sum(grad_K,dim=0).shape)
    # print("torch.sum(K_XX,dim=0)",torch.sum(K_XX,dim=0).reshape(-1,1).shape)

    # TODO:
    # check dimensions of the torch.sum()
    # recheck signs
